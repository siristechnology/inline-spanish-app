...
import { List, ListProps } from 'react-native-ui-kitten/ui'
...
class CommentList1Component extends React.Component<CommentsList1Props> {
	...
	private renderItem = (info: ListItemElementInfo): ListItemElement => {
		...
		const listItemElement: ListItemElement = this.renderListItemElement(
			item,
		)

		const additionalStyle: ViewStyle = this.isLastItem(index)
			? null
...
export const CommentsList1 = withStyles(
	CommentList1Component,
	(theme: ThemeType) => ({
		item: {
